...
function calcularTotalApesentacao(pecas, apre) {
  let total = 0;
...
function calcularTotalFatura(pecas, apresentacoes) {
  let totalFatura = 0;
  for (let apre of apresentacoes) {
    totalFatura += calcularTotalApesentacao(pecas, apre);
  }
  return totalFatura;
}
...
function gerarFaturaStr (fatura, pecas) {
  ...
  for (let apre of fatura.apresentacoes) {
    faturaStr += `  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calcularTotalApesentacao(pecas, apre))} (${apre.audiencia} assentos)\n`;
  }
...
function gerarFaturaHTML(fatura, pecas) {
  ...
  for (let apre of fatura.apresentacoes) {
    faturaHTML += `<li>  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calcularTotalApesentacao(pecas, apre))} (${apre.audiencia} assentos) </li>\n`;
  }
  faturaHTML += `</ul>\n`;
