...
class ServicoCalculoFatura {  
  ...
  calcularTotalApesentacao(pecas, apre) {
    let total = 0;
  ...
  calcularTotalFatura(pecas, apresentacoes) {
    let totalFatura = 0;
    for (let apre of apresentacoes) {
      totalFatura += this.calcularTotalApesentacao(pecas, apre);
    }
    return totalFatura;
  }
...
function gerarFaturaStr (fatura, pecas, calc) {
  ...
  for (let apre of fatura.apresentacoes) {
    faturaStr += `  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calc.calcularTotalApesentacao(pecas, apre))} (${apre.audiencia} assentos)\n`;
  }
...
//   for (let apre of fatura.apresentacoes) {
//     faturaHTML += `<li>  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calc.calcularTotalApesentacao(pecas, apre))} (${apre.audiencia} assentos) </li>\n`;
//   }
//   faturaHTML += `</ul>\n`;
